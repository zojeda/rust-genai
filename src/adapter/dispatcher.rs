...
impl Adapter for AdapterDispatcher {
	async fn all_model_names(kind: AdapterKind) -> Result<Vec<String>> {
		match kind {
			...
			_ => OpenAIAdapter::all_model_names(kind).await,
	...
	fn default_adapter_config(kind: AdapterKind) -> &'static AdapterConfig {
		match kind {
			...
			_ => OpenAIAdapter::default_adapter_config(kind),
	...
	fn get_service_url(model_info: ModelInfo, service_type: ServiceType) -> String {
		match model_info.adapter_kind {
			...
			_ => OpenAIAdapter::get_service_url(model_info, service_type),
	...
	fn to_web_request_data(
		...
		match model_info.adapter_kind {
			...
			_ => OpenAIAdapter::to_web_request_data(model_info, config_set, service_type, chat_req, options_set), // AdapterKind::Anthropic => {
	...
	fn to_chat_stream(
		...
		match model_info.adapter_kind {
			...
			_ => OpenAIAdapter::to_chat_stream(model_info, reqwest_builder, options_set),
